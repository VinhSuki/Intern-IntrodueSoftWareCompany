...
const BannerBreadCrumb = ({ title = '' }) => {
  return (
    <div className='xl:mt-[100px] lg:mt-[90px] md:mt-[80px] sm:mt-[70px] mt-[60px] bg-transparent xl:h-[400px] lg:h-[350px] md:h-[300px] sm:h-[250px] h-[200px] relative z-20 flex items-center flex-col justify-center font-blinker gap-6'>
      ...
      <p className="xl:text-xl lg:text-lg md:text-base sm:text-sm text-xs font-bold text-white"><Link to={'/'} className="text-third hover:text-hover transition">Home</Link><span> / </span><span>{title}</span></p>
    </div>
  )
}
